...
async def main():
    async with async_playwright() as p:
        ...
        pg = await b.new_page()
        await pg.goto('file://' + str(pathlib.Path(__file__).parent / 'how-it-works.html')); await pg.wait_for_timeout(300)
        ...
        await pg.pdf(path=str(pathlib.Path(__file__).parent / 'Margin-how-it-works.pdf'), format='A4', print_background=True, margin={'top':'0','bottom':'0','left':'0','right':'0'}, prefer_css_page_size=True)
        await b.close()
asyncio.run(main())
